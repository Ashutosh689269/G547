...
#define Mem_Size 1024 // Size of Ram disk
# define k_size (512)
...
static int myopen(struct block_device *x, fmode_t mode)	 
{
	int ret=0;
...
static void myrelease(struct gendisk *disk, fmode_t mode)
{
...
{
	(device.data) = vmalloc(Mem_Size*sect_size);
...
static int transfer_rb(struct request *req)
{
	...
	rq_for_each_segment(bv, req, iter)
	{
		...
		if (dir == WRITE) /* Write to the device */
		{
			...
			memcpy((device.data)+((start_sector+sector_offset)*sect_size)\
			,buffer,sectors*sect_size);		
...
static void request_dev(struct request_queue *q)
{
	struct request *req;
	int error;
						      ...
						      *for data transfer*/
	{
		...
		error=transfer_rb(req);// transfer the request for operation
...
void device_setup(void)
{
	...
	device.queue = blk_put_queue( request_dev, &device.lock); 
	...
	set_capacity(device.gd, Mem_Size*(k_size/sect_size));  
	add_disk(device.gd);
